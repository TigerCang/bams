...
class DirectBudget extends BaseController
{
    ...
    public function index()
    {
        // checkPage('119');
        $data = [
            't_title' => lang('app.direct budget'),
            't_span' => lang('app.span direct budget'),
            'link' => base_url('directbudget'),
    ...
    public function inputData()
    {
        ...
        $data = [
            ...
            'selectObject' => $this->mainModel->distSelect('object'),
            'selectIndustry' => $this->mainModel->distSelect('group industry'),
        ...
        $this->render('general/budget/directBudget_input', $data);
    }
    ...
    public function addData()
    {
        if ($this->request->isAJAX()) {
            ...
            $validationRules = [
                ...
                'totalContract' => ['rules' => 'required', 'errors' => ['required' => lang("app.err blank")]],
                'totalWork' => ['rules' => 'required', 'errors' => ['required' => lang("app.err blank")]],
            ];
            ...
            if (!$this->validate($validationRules)) {
                $msg = [
                    'error' => [
                        ...
                        'totalContract' => $this->validation->getError('totalContract'),
                        'totalWork' => $this->validation->getError('totalWork'),
                    ]
                ];
            } else {
                // Save
                ...
                if (empty($db1)) {
                    $this->budgetParentModel->save([
                        ...
                        'object_id' => $branch,
                        'segment_id' => $this->request->getVar('xSegment'),
                ...
                $this->budgetChild1Model->save([
                    ...
                    'price_contract' => changeSeparator($this->request->getVar('priceContract')),
                    'price_work' => changeSeparator($this->request->getVar('priceWork')),
                    'total_contract' => changeSeparator($this->request->getVar('totalContract')),
                    'total_work' => changeSeparator($this->request->getVar('totalWork')),
                    'group_industry' => $this->request->getVar('groupIndustry'),
                    'notes' => $this->request->getVar('notes'),
                ]);
                $insertedId = $this->budgetChild1Model->getInsertID();
                $this->budgetChild1Model->update($insertedId, ['budget_id' => $insertedId]);

                $db3 = $this->mainModel->getParentCost('direct', $this->request->getVar('cost'));
                $cekLevel2 = $this->transModel->cekBudgetChild($dbParent[0]->id, 'cost_id', $db3[0]->idLevel2);
                if ($cekLevel2) {
                    $total2 = $this->transModel->budgetTotal('cost', $dbParent[0]->id, $db3[0]->idLevel2);
                    $this->budgetChild1Model->save(['id' => $cekLevel2[0]->id, 'total_contract' => $total2[0]->totalcontract, 'total_work' => $total2[0]->totalwork]);
                    $this->mainModel->updateDeletedAt('budget_child1', $cekLevel2[0]->id);
                } else {
                    $this->budgetChild1Model->save([
                        'parent_id' => $dbParent[0]->id,
                        'cost_id' => $db3[0]->idLevel2,
                        'total_contract' => changeSeparator($this->request->getVar('totalContract')),
                        'total_work' => changeSeparator($this->request->getVar('totalWork')),
                    ]);
                }

                $cekLevel1 = $this->transModel->cekBudgetChild($dbParent[0]->id, 'cost_id', $db3[0]->idLevel1);
                if ($cekLevel1) {
                    $total1 = $this->transModel->budgetTotal('cost', $dbParent[0]->id, $db3[0]->idLevel1);
                    $this->budgetChild1Model->save(['id' => $cekLevel1[0]->id, 'total_contract' => $total1[0]->totalcontract, 'total_work' => $total1[0]->totalwork]);
                    $this->mainModel->updateDeletedAt('budget_child1', $cekLevel1[0]->id);
                } else {
                    $this->budgetChild1Model->save([
                        'parent_id' => $dbParent[0]->id,
                        'cost_id' => $db3[0]->idLevel1,
                        'total_contract' => changeSeparator($this->request->getVar('totalContract')),
                        'total_work' => changeSeparator($this->request->getVar('totalWork')),
                        'level' => '1',
                    ]);
                ...
                if ($this->request->getVar('unique') == '') {
                    $msg = [
                        'redirect' => '/directbudget/input?search=' . $unique,
                        'message' => "{$db3[0]->name}" . lang('app.title add'),
                    ];
                } else {
                    $msg = ['message' => "{$db3[0]->name}" . lang('app.title add')];
                }
            }
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $segment = ($db1[0]->status == '0' ? $this->request->getVar('segment') : $db1[0]->segment_id);
    ...
    public function updateData()
    {
        if ($this->request->isAJAX()) {
            ...
            $total = $this->transModel->budgetTotal('account', $dbParent[0]->id, '', '1');
            $this->mainModel->updateData('budget_parent', 'total', $total[0]->totalcontract, 'unique', $dbParent[0]->unique);
    ...
    public function deleteData()
    {
        if ($this->request->isAJAX()) {
            ...
            $total = $this->transModel->budgetTotal('account', $dbParent[0]->id, '', '1');
            $nTotal = (!empty($total) ? $total[0]->totalcontract : '0');
            $this->mainModel->updateData('budget_parent', 'total', $nTotal, 'unique', $dbParent[0]->unique);
    ...
    public function outFocusProject()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = ['categoryProject' => $file['categoryProject'], 'segment' => $segmentData];
            return $this->response->setJSON($msg);
        } else {
            exit('out');
        }
    ...
    public function tableData()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = ['data' => view('x-general/directBudget_table', $data)];
            return $this->response->setJSON($msg);
        } else {
            exit('out');
        }
