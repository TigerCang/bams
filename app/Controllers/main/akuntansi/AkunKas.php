...
class AkunKas extends BaseController
{
    ...
    public function index()
    {
        checkPage('102');
        $data = [
            't_judul' => lang('app.kas bank'),
            't_span' => lang('app.span kas bank'),
            'link' => '/akunkas',
            'khid' => 'hidden',
            'phid' => '',
            'nhid' => 'hidden',
            'kelakun' => $this->mainModel->getKelakun($this->urls[1], 'kas'),
        ];
    ...
    public function inputData()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                't_modal' => lang('app.kas bank'),
                'link' => "/akunkas",
                'selkel' => $this->mainModel->distSelect('akun kas', 't'),
                'selnama' => $this->mainModel->distSelect('akun kas'),
