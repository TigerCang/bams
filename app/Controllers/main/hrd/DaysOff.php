...
class DaysOff extends BaseController
{
    protected $fileModel;
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $cekData = $this->mainModel->cekData('m_file', 'name', $this->request->getVar('description'), 'param', 'days off', $this->request->getVar('unique'));
            $ruleName = ($cekData ? 'required|is_unique[m_file.name]' : 'required');
            $cekData2 = $this->mainModel->cekData('m_file', 'name2', $this->request->getVar('code'), 'param', 'days off', $this->request->getVar('unique'));
            $ruleCode = ($cekData2 ? 'required|is_unique[m_file.name2]' : 'required');
            ...
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postAction') == 'save') {
                    ...
                    $this->fileModel->save([
                        ...
                        'name2' => strtoupper($this->request->getVar('code')),
                        'value' => $this->request->getVar('day'),
                        'set_default' => ($this->request->getVar('cutoff') == 'on' ? '1' : '0'),
