...
class NomorDokumen extends BaseController
{
    ...
    public function index()
    {
        checkPage('102');
        $data = [
            't_judul' => lang('app.kode dokumen'),
            't_span' => lang('app.span kode dokumen'),
            'link' => '/nodokumen',
            'phid' => 'hidden',
            'form' => $this->mainModel->getForm($this->urls[1], 'dokumen', '', ''),
        ];
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $rule_kode = (strlen($this->request->getVar('kode')) > "3" ? 'valid_email' : 'required');
            $cekForm = $this->mainModel->cekForm('dokumen', $this->request->getVar('form'), $this->request->getVar('idunik'), '', '');
            $rule_form = ($cekForm ? 'valid_email' : 'permit_empty');
            $idunik = $this->request->getVar('idunik');

            $validationRules = [
                'kode' => ['rules' => $rule_kode, 'errors' => ['required' => lang("app.err blank"), 'valid_email' => lang("app.err unik")]],
            ...
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postaction') == 'save') {
                    ...
                    $this->berkasModel->save([
                        ...
                        'nama' => strtoupper($this->request->getVar('kode')),
                        'kondisi' => $kondisi,
