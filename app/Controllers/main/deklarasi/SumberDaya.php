...
class SumberDaya extends BaseController
{
    ...
    public function index()
    {
        checkPage('102');
        $data = [
            't_judul' => lang('app.sumber daya'),
            't_span' => lang('app.span sumberdaya'),
            'link' => '/sumberdaya',
            'asal' => 'sumber daya',
            'kategori' => $this->mainModel->distinctBiaya('sumber daya', '1'),
        ];
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $akun = ($level == "2" ? $this->request->getVar('xakun') : '');
            ...
            if (strlen($this->request->getVar('kode')) == "8") {
                if ($level != "1") {
                    $cekInduk = $this->mainModel->cekIndukbiaya('sumber daya', strtoupper($biayaInduk), $levelInduk, '');
            ...
            $validationRules = [
                ...
                'satuan' => ['rules' => $rule_lev2, 'errors' => ['required' => lang("app.err pilih")]],
                'akun' => ['rules' => $rule_lev2, 'errors' => ['required' => lang("app.err pilih")]],
            ];
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postaction') == 'save') {
                    ...
                    $this->biayaModel->save([
                        ...
                        'level' => $level,
                        'akun_id' => $akun,
                        'is_jumlah' => ($this->request->getVar('voljl') == 'on' ? '1' : '0'),
                        'kondisi' => $kondisi,
