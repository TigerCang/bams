...
class Satuan extends BaseController
{
    ...
    public function index()
    {
        checkPage('102');
        $data = [
            't_judul' => lang('app.satuan'),
            't_span' => lang('app.span satuan'),
            'link' => '/satuan',
            'satuan' => $this->mainModel->getBerkas($this->urls[1], 'satuan'),
            'chid' => 'hidden',
            'ghid' => 'hidden'
        ];
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $nama = $this->mainModel->cekBerkas('satuan', 'nama', $this->request->getVar('deskripsi'), $this->request->getVar('idunik'));
