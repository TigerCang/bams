...
class KateProyek extends BaseController
{
    ...
    public function index()
    {
        checkPage('102');
        $data = [
            't_judul' => lang('app.kategori proyek'),
            't_span' => lang('app.span kategori proyek'),
            'link' => '/kateproyek',
            'chid' => 'hidden',
            'ghid' => 'hidden',
            'satuan' => $this->mainModel->getBerkas($this->urls[1], 'kategori proyek'),
        ];
    ...
    public function inputData()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                't_modal' => lang('app.kategori proyek'),
                'link' => "/kateproyek",
                'chid' => 'hidden',
                'ghid' => 'hidden',
                'param' => 'kategori proyek',
                'perusahaan' => [],
                'wilayah' => [],
                'divisi' => [],
                'berkas' => $db1,
    ...
    public function saveData()
    {
        if ($this->request->isAJAX()) {
            ...
            $nama = $this->mainModel->cekBerkas('kategori proyek', 'nama', $this->request->getVar('deskripsi'), $this->request->getVar('idunik'));
