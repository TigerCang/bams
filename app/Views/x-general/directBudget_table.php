<table id="tableData" class="table table-striped table-hover">
    <thead>
        <tr class="tr-color">
            ...
            <th width="250" rowspan="2" class="align-middle"><?= lang('app.cost') ?></th>
            <th rowspan="2" class="align-middle"><?= lang('app.description') ?></th>
            <th width="100" rowspan="2" class="align-middle text-end"><?= lang('app.quantity') ?></th>
            <th colspan="2" class="text-center"><?= lang('app.price') ?></th>
            <th colspan="2" class="text-center"><?= lang('app.total') ?></th>
            <th rowspan="2" class="align-middle"><?= lang('app.group') ?></th>
            <th rowspan="2" class="align-middle"><?= lang('app.notes') ?></th>
            <th width="5" rowspan="2" class="align-middle" data-orderable="false"></th>
        </tr>
        <tr class="tr-color">
            <th width="150" class="text-end"><?= lang('app.contract') ?></th>
            <th width="150" class="text-end"><?= lang('app.work') ?></th>
            <th width="150" class="text-end"><?= lang('app.contract') ?></th>
            <th width="150" class="text-end"><?= lang('app.work') ?></th>
        </tr>
    </thead>
    <tbody>
        <?php foreach ($budget as $index => $row) :
            ...
            <tr class="<?= $label['class'] ?>">
                ...
                <td class="text-end"><?= ($row->level == '3' ? formatComa($row->quantity, '4') : '') ?></td>
                <td class="text-end"><?= ($row->level == '3' ? formatComa($row->price_contract) : '') ?></td>
                <td class="text-end"><?= ($row->level == '3' ? formatComa($row->price_work) : '') ?></td>
                <td class="text-end"><?= formatComa($row->total_contract) ?></td>
                <td class="text-end"><?= formatComa($row->total_work) ?></td>
                <td><?= ($row->level == '3' ? lang('app.' . $row->group_industry) : '') ?></td>
                <td><?= $row->notes ?></td>
                <td>
                    <?php if ($row->level == '3') : ?>
                        <div class="dropdown">
