            ...
            <div class="card-datatable table-responsive">
                <table id="tableInit" class="table table-striped table-hover">
                    <thead>
                        <tr class="tr-color">
                            <th width="5">#</th>
                            <th width="200"><?= lang('app.serial') ?></th>
                            <th><?= lang('app.item') ?></th>
                            <th width="150"><?= lang('app.price') ?></th>
                            <th width="150"><?= lang('app.equipment') ?></th>
                            <th width="100" class="text-center"><?= json('status') ?></th>
                            <th width="5" data-orderable="false"></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($serial as $index => $row) :
                            $label = labelBadge('main', ($row->adaptation)) ?>
                            <tr <?= ($row->xLog == '' ? 'class="fw-bold"' : '') ?>>
                                <td><?= $index + 1 ?>.</td>
                                <td><?= $row->serial ?></td>
                                <td><?= $row->itemName ?></td>
                                <td><?= formatComa($row->price) ?></td>
                                <td><?= $row->toolNumber ?></td>
