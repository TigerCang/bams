                ...
                <div class="row">
                    <div class="col-12 col-md-12 col-lg-6 d-flex justify-content-between align-items-center mb-4">
                        <div class="form-floating form-floating-outline w-100 me-4">
                            <select class="select2-non form-select" id="skategori" name="skategori" data-allow-clear="true" data-placeholder="<?= lang('app.pilih-') ?>">
                                <?php foreach ($selkategori as $db) : ?>
                                    <option value="<?= $db->nama ?>" <?= (session()->getFlashdata('flash-kate') == $db->nama ? 'selected' : '') ?>><?= lang('app.' . $db->nama) ?></option>
                                <?php endforeach ?>
                            </select>
