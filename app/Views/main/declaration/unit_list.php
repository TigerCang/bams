            ...
            <div class="card-datatable table-responsive">
                <table id="tableInit" class="table table-striped table-hover nowrap">
                    <thead>
                        <tr class="tr-color">
                            <th width="5">#</th>
                            <th width="200"><?= lang('app.param') ?></th>
                            <th width="150" <?= $cHid ?>><?= lang('app.code') ?></th>
                            <th><?= lang('app.description') ?></th>
                            <th width="200" <?= $rHid ?>><?= lang('app.company') ?></th>
                            <th width="100" class="text-center" <?= $cHid ?>><?= lang('app.day') ?></th>
                            <th width="100" class="text-center"><?= json('status') ?></th>
                            <th width="5" data-orderable="false"></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($unit as $index => $row) :
                            $label = labelBadge('main', ($row->adaptation)) ?>
                            <tr <?= ($row->xLog == '' ? 'class="fw-bold"' : '') ?>>
                                <td><?= $index + 1 ?>.</td>
                                <td><?= lang('app.' . $row->param) ?></td>
                                <td <?= $cHid ?>><?= $row->name2 ?></td>
                                <td><?= $row->name ?></td>
                                <td <?= $rHid ?>><span><?= $row->company ?></span>
                                    <small class="text-truncate mb-0 d-none d-sm-block"><?= $row->division ?></small>
                                </td>
                                <td class="text-center" <?= $cHid ?>><?= $row->value ?></td>
