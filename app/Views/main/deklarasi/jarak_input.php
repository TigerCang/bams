                ...
                <div class="row g-2">
                    <div class="col-12 col-md-6 col-lg-6 mb-4">
                        <div class="form-floating form-floating-outline">
                            <input type="text" <?= (isset($ruas[0]->kondisi[0]) && $ruas[0]->kondisi[0] == '1' ? 'readonly' : '') ?> class="form-control text-uppercase" id="kode" name="kode" maxlength="6" placeholder="<?= lang('app.harus') ?>" value="<?= ($ruas[0]->kode ?? '') ?>" />
                    ...
                    <div class="col-12 col-md-6 col-lg-6 mb-4">
                        <div class="form-floating form-floating-outline">
                            <input type="text" class="form-control autonumber" data-a-sep="." data-a-dec="," id="jarak" name="jarak" placeholder="" value="<?= ($ruas[0]->jarak ?? '') ?>" />
                            <label for="jarak"><?= lang('app.jarak') . ' (Km)' ?></label>
                            <div id="error" class="invalid-feedback errjarak"></div>
                        </div>
                    </div>
                </div>
                <div class="row">
    ...
    $('.btnsave').click(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#kode, #deskripsi, #jarak').removeClass('is-invalid');
                $('.errkode, .errdeskripsi, .errjarak').html('');
                if (response.error) {
                    handleFieldError('kode', response.error.kode);
                    handleFieldError('deskripsi', response.error.deskripsi);
                    handleFieldError('jarak', response.error.jarak);
                } else {
