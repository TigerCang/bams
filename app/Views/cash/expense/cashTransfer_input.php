...
<div class="row g-2">
        ...
        <div class="card mb-6">
            <div class="card-body">
                <div class="row g-2">
                    <div class="col-12 col-md-8 col-lg-8 mb-2">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-non form-select" id="company" name="company" <?= (isset($cash[0]->status) && $cash[0]->status >= '1' ? 'disabled' : '') ?>>
                ...
                <div class="row g-2" id="zBranch">
                    ...
                    <div class="col-12">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-non form-select" id="branch" name="branch" <?= (isset($cash[0]->status) && $cash[0]->status >= '1' ? 'disabled' : '') ?> data-allow-clear="true" data-placeholder="<?= lang('app.selectSearch') ?>">
                                <?php if ($object1) : ?>
    ...
    <div class="col-12 col-md-12 col-lg-6">
        <div class="card mb-6">
            <div class="card-body">
                <input type="hiddena" id="unique" name="unique" value="<?= ($cash[0]->unique ?? '') ?>" />
                <input type="hiddena" id="xObject" name="xObject" value="<?= ($cash[0]->object ?? 'branch') ?>" />
                <div class="row g-2">
                    <div class="col-12 col-md-8 col-lg-8 mb-2">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-non form-select" id="requester" name="requester" <?= (isset($cash[0]->status) && $cash[0]->status >= '1' ? 'disabled' : '') ?> data-allow-clear="true" data-placeholder="<?= lang('app.select-') ?>">
                                <?php if ($requester1) : ?><option value="<?= $requester1[0]->id ?>" selected><?= "{$requester1[0]->code} &ensp;&emsp; {$requester1[0]->employeeName}" ?></option><?php endif ?>
                            </select>
                    ...
                    <div class="col-12 mb-2">
                        <div class="form-floating form-floating-outline">
                            ...
                            <select class="select2-non form-select" id="person" name="person" <?= (isset($cash[0]->status) && $cash[0]->status >= '1' ? 'disabled' : '') ?> data-allow-clear="true" data-placeholder="<?= lang('app.select-') ?>" onchange="clickPerson()">
                                <?php if ($person1) : ?><option value="<?= $person1[0]->id ?>" selected><?= "{$person1[0]->code} &ensp;&emsp; {$person1[0]->name}" ?></option><?php endif ?>
                            </select>
        ...
        <div class="card mb-6">
            <div class="card-body">
                <div class="row g-2">
                    <div class="col-12 mb-2">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-non form-select" id="account" name="account" <?= (isset($cash[0]->status) && $cash[0]->status >= '1' ? 'disabled' : '') ?> data-allow-clear="true" data-placeholder="<?= lang('app.select-') ?>">
                                <?php foreach ($selectAccount as $db) : ?>
                                    <option value="<?= $db->accountID ?>" <?= (isset($cash[0]) && $cash[0]->account_id == $db->accountID ? 'selected' : '') ?>><?= "{$db->groupName}  ->  {$db->accountCode} &ensp;&emsp; {$db->accountName}" ?></option>
                                <?php endforeach ?>
                            </select>
            ...
            <div class="card-footer">
                <div class="row w-100">
                    ...
                    <div class="col-12 col-md-6 col-lg-6 ms-auto text-end">
                        <div class="btn-group" id="dropdown-icon" <?= $button['off'] ?>>
...
<script>
    let isPageLoaded;
    $("#company, #region, #division, #object").change(function() {
    ...
    $(document).ready(function() {
        isPageLoaded = true;
        $("#company, #region, #division, #object").trigger("change")
    ...
    function submitForm(getAction) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#askAccess, #company, #region, #division, #branch, #requester, #person, #attachment, #account, #total').removeClass('is-invalid');
                $('.err_askAccess, .err_company, .err_region, .err_division, .err_branch, .err_requester, .err_person, .err_attachment, .err_account, .err_total').html('');
                if (response.error) {
