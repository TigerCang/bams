        ...
        <div class="card mb-6">
            <div class="card-body">
                <div class="row g-2">
                    <div class="col-12 mb-4">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-subtext form-select" id="akun" name="akun" data-placeholder="<?= lang('app.pilih-') ?>">
                            </select>
                ...
                <div class="row g-2">
                    <div class="col-12 col-md-6 col-lg-6 mb-4">
                        <div class="form-floating form-floating-outline">
                            <select class="select2-subtext form-select" id="supir" name="supir" data-allow-clear="true" data-placeholder="<?= lang('app.pilihsr') ?>">
                            </select>
                            <label for="supir"><?= lang('app.supir') . '(' . lang('app.pegawai') . ')' ?></label>
                            <div id="error" class="invalid-feedback errsupir"></div>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 col-lg-6 mb-4">
                        <div class="form-floating form-floating-outline">
                            <input type="text" class="form-control" id="supir2" name="supir2" placeholder="" value="" />
                            <label for="supir2"><?= lang('app.supir') . '(' . lang('app.rekan') . ')' ?></label>
                            <div id="error" class="invalid-feedback errsupir2"></div>
                        </div>
                    </div>
                </div>
                <div class="row g-2">
...
<script>
    ...
    function klikPenerima() {
        var getPenerima = $("#penerima").val();
        $.ajax({
            type: "POST",
            url: "<?= $link ?>/klikpenerima",
            data: {
                penerima: getPenerima,
            },
            dataType: "json",
            success: function(response) {
                if (response.akun) $("#akun").html(response.akun);
            },
            error: function(xhr, ajaxOptions, thrownError) {
                alert(xhr.status + "\n" + xhr.responseText);
                alert(thrownError);
    ...
    $('.btnsave').click(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#kodebeban, #penerima, #lampiran, #akun').removeClass('is-invalid');
                $('.errkodebeban, .errpenerima, .errlampiran, .errakun').html('');
                if (response.error) {
